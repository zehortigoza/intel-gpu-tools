...
#define ADD_PROPS(_head, _tail, _key, _value) \
	do { \
		igt_assert((_tail - _head) < (DRM_XE_OA_PROP_MAX * 2)); \
		*_tail++ = DRM_XE_OA_PROP_##_key; \
		*_tail++ = _value; \
	} while (0)
...
__perf_open(int fd, struct drm_xe_oa_open_param *param, bool prevent_pm)
{
	...
	ret = xe_perf_ioctl(fd, DRM_IOCTL_XE_PERF_OPEN, param);
...
test_system_wide_paranoid(void)
{
	igt_fork(child, 1) {
		...
		xe_perf_ioctl_err(drm_fd, DRM_IOCTL_XE_PERF_OPEN, &param, EACCES);
	}
...
test_invalid_open_flags(void)
{
	...
	xe_perf_ioctl_err(drm_fd, DRM_IOCTL_XE_PERF_OPEN, &param, EINVAL);
}
...
gen12_test_mi_rpc(const struct intel_execution_engine2 *e)
{
	...
	uint64_t properties[] = {
		 ...
		 * measured
		 */
		DRM_XE_OA_PROP_EXEC_QUEUE_ID, UINT64_MAX,

		/* OA unit configuration:
		 * DRM_XE_OA_PROP_SAMPLE_OA is no longer required for Gen12
...
{
	int ret = xe_perf_ioctl(fd, DRM_IOCTL_XE_PERF_ADD_CONFIG, config);
	if (ret < 0)
		ret = -errno;
	return ret;
}
...
{
	igt_assert_eq(xe_perf_ioctl(fd, DRM_IOCTL_XE_PERF_REMOVE_CONFIG, &config_id), 0);
}

static bool has_i915_perf_userspace_config(int fd)
{
	uint64_t config = 0;
	int ret = xe_perf_ioctl(fd, DRM_IOCTL_XE_PERF_REMOVE_CONFIG, &config);
	igt_assert_eq(ret, -1);
...
test_invalid_remove_userspace_config(void)
{
	...
	igt_fork(child, 1) {
		...
		xe_perf_ioctl_err(drm_fd, DRM_IOCTL_XE_PERF_REMOVE_CONFIG, &config_id, EACCES);
	}
	igt_waitchildren();

	/* Removing invalid config ID should fail. */
	xe_perf_ioctl_err(drm_fd, DRM_IOCTL_XE_PERF_REMOVE_CONFIG, &wrong_config_id, ENOENT);
...
test_create_destroy_userspace_config(void)
{
	...
	uint64_t config_id;
	uint64_t properties[] = {
		DRM_XE_OA_PROP_OA_METRICS_SET, 0, /* Filled later */
...
test_whitelisted_registers_userspace_config(void)
{
	...
	/* Create a new config */
	ret = xe_perf_ioctl(drm_fd, DRM_IOCTL_XE_PERF_ADD_CONFIG, &config);
...
test_group_exclusive_stream(const intel_ctx_t *ctx, bool exponent)
{
	...
	for (i = 0; i < num_perf_oa_groups; i++) {
		...
		if (!exponent) {
			properties[0] = DRM_XE_OA_PROP_EXEC_QUEUE_ID;
			properties[1] = ctx->id;
		}
		...
		grp->perf_fd = xe_perf_ioctl(drm_fd,
	...
	for (i = 0; i < num_perf_oa_groups; i++) {
		...
		for (j = 0; j < grp->num_engines; j++) {
			...
			xe_perf_ioctl_err(drm_fd, DRM_IOCTL_XE_PERF_OPEN, &param, EBUSY);
			...
			errno = 0;
			err = xe_perf_ioctl(drm_fd, DRM_IOCTL_XE_PERF_OPEN, &param);
			igt_assert(err < 0);
