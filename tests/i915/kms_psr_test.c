...
static void setup_plane_on_off(data_t *data)
{
	...
	data->overlay = igt_output_get_plane_type(data->output,
						  DRM_PLANE_TYPE_OVERLAY);
	igt_plane_set_fb(data->primary, &data->fbs[0]);
	igt_plane_set_fb(data->overlay, &data->fbs[1]);
...
static void run_plane_on_off(data_t *data)
{
...
igt_main_args("", long_options, help_str, opt_handler, &_data)
{
	...
	igt_subtest("plane_on_off") {
		setup_plane_on_off(data);
		run_plane_on_off(data);
		teardown(data);
	}
